...
class VxSerialBaseDebuger(VxSerialBaseTarget):
    ...
    def is_task_on_break_point(self, task):
        ...
        if self._is_task_in_debug_loop(task):
            ...
            if self.endian == 2:
                pack_parm = "<I"
            sp = int(self.current_task_regs[task]["r13"], 16)
            break_point = (struct.unpack(pack_parm, self.get_mem_dump(sp + 0x08, 0x04))[0] - 0) & 0xfffffffe # take 16bit in considerate
        ...
        for task in current_tasks_status:
            ...
            try:
                if self._is_address_in_debug_loop(int(current_tasks_status[task]['pc'], 16)):
                    return current_tasks_status[task]["tid"]
            except:
                return None
        return None
    ...
    def show_task_stack(self, regs):
        """Display task stack data

        :param regs: Task regs
        :return:
        """
    ...
    def show_task_bp_asm(self, task):
        ...
        bp_asm_code = self.disassemble(bp_asm_data, bp_address, CS_ARCH_ARM, CS_MODE_ARM,CS_MODE_BIG_ENDIAN if self.endian == 1 else  CS_MODE_LITTLE_ENDIAN)
        print(bp_asm_code)
    ...
    def show_task_bp_context(self, task):
        ...
        regs = self.show_task_bp_regs(task)
        # stack
        print("{:-^{width}}".format("stack", width=80))
        self.show_task_stack(regs)
        # asm
    ...
    def add_break_point(self, bp_address, is_16bit = 0,bp_type=0, condition=None):
        ...
        bp_asm_code = self.disassemble(original_asm, bp_address, CS_ARCH_ARM, CS_MODE_ARM,CS_MODE_BIG_ENDIAN if self.endian == 1 else CS_MODE_LITTLE_ENDIAN)
